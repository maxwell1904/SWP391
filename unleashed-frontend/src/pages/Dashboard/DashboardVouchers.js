...
const DashboardVouchers = () => {
  ...
  useEffect(() => {
    apiClient
      .get("/api/vouchers/voucher-statuses", {
        headers: { Authorization: varToken },
      })
      .then((res) => setStatuses(res.data));
    apiClient
      .get("/api/vouchers/voucher-types", {
        headers: { Authorization: varToken },
      })
  ...
  const handleDelete = () => {
    apiClient
      .delete(`/api/vouchers/${voucherToDelete.voucherId}`, {
        headers: { Authorization: varToken },
      })
      .then(() => {
        fetchVouchers();
  ...
  return (
    <div className="p-4">
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">
                    {d.voucherTypeName === "PERCENTAGE"
                      ? `${d.voucherValue}%`
                      : formatPrice(d.voucherValue)}
                  </td>
                  <td className="px-4 py-3 text-sm">
                    <Chip
                      label={d.voucherStatusName}
                      color={getStatusChipColor(d.voucherStatusName)}
                      size="small"
                    />
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <Tooltip title="View Details">
                        <IconButton
                          component={Link}
                          to={`/Dashboard/Vouchers/${d.voucherId}`}
                          color="success"
                          size="small"
                        >
                          <Visibility />
                      ...
                      <Tooltip title="Edit Voucher">
                        <IconButton
                          component={Link}
                          to={`/Dashboard/Vouchers/Edit/${d.voucherId}`}
                          color="primary"
                          size="small"
                        >
                          <Edit />
                      ...
                      <Tooltip title="Assign to Users">
                        <IconButton
                          component={Link}
                          to={`/Dashboard/Vouchers/${d.voucherId}/Assign`}
                          color="secondary"
                          size="small"
                        >
                          <PersonAdd />
      ...
      <DeleteConfirmationModal
        ...
        name={voucherToDelete?.voucherCode || ""}
      />
    </div>
  );
};
