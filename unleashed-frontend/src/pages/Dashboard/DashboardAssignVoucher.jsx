...
const DashboardAssignVoucher = () => {
    ...
    useEffect(() => {
        const fetchVoucherDetails = async () => {
            try {
                const res = await apiClient.get(`/api/vouchers/${voucherId}`, { headers: { Authorization: varToken } });
                setVoucher(res.data);
            } catch (error) {
        ...
        const fetchAssignedUsers = async () => {
            try {
                const res = await apiClient.get(`/api/vouchers/${voucherId}/users`, { headers: { Authorization: varToken } });
        ...
        Promise.all([fetchVoucherDetails(), fetchAssignedUsers()]);
    }, [voucherId, varToken]);
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            await apiClient.post(`/api/vouchers/${voucherId}/users`, newAssignments, {
                headers: { Authorization: varToken }
            });
            toast.success(`${newAssignments.length} user(s) have been assigned the voucher.`);
    ...
    return (
        <Container maxWidth="lg" sx={{ p: 4 }}>
            <Typography variant="h4" gutterBottom fontWeight="bold">
                Assign Users to Voucher
            </Typography>

            {voucher && (
                <Alert severity="info" sx={{ mb: 3 }}>
                    You are assigning users to the voucher code: <strong>{voucher.voucherCode}</strong> ({voucher.voucherTypeName}).
                </Alert>
            )}
            ...
            <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
                    ...
                    <Button variant="outlined" onClick={() => navigate('/Dashboard/Vouchers')}>
                        Cancel
                    </Button>
                </Box>
            </Paper>
...
export default DashboardAssignVoucher;