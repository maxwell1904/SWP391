...
const getShippingMethodFee = (method) => {
  if (!method) return 0;
  return method.id === 1 ? 20000 : 10000;
};
...
const CheckoutPage = () => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        setUserData({
          ...
          userAddress:
            userInfo.userAddress === "Address not provided"
              ? ""
        ...
        if (fullAddress && fullAddress !== "Address not provided") {
          ...
          if (addressParts.length >= 4) {
            const houseAndStreet = addressParts
  ...
  const totalQuantity = useMemo(
    () => itemCheckout.reduce((total, item) => total + item.quantity, 0),
    [itemCheckout],
  );
  ...
  const handlePaymentMethodDes = (name) => {
    ...
    if (name === "TRANSFER")
      return "Transfer the total amount to our bank account.";
    return "";
  };
      ...
      <div className="checkoutInfomation w-full">
        <div className="bgCartItem bg-gray-100 md:px-9">
          ...
          <List sx={style}>
            {itemCheckout.map((item) => (
              <React.Fragment key={item.id}>
                ...
                >
                  <ListItemIcon sx={{ minWidth: 82, mt: 0.5 }}>
          ...
          >
            ...
            >
              <Box>
                <Typography fontWeight="600">Items in order</Typography>
                <Typography variant="body2" color="text.secondary">
                  {itemCheckout.length} product(s) | {totalQuantity} item(s)
                </Typography>
              </Box>
              <Typography fontWeight="700">{formatPrice(cartTotal)}</Typography>
            </Box>
            <Box
            ...
            >
              ...
              <Typography
                fontWeight="600"
                color={shippingMethod ? "text.primary" : "text.secondary"}
              >
                {shippingMethod
                  ? formatPrice(shippingMethodFee)
                  : "Select service"}
              </Typography>
            </Box>
            <Box
      ...
      <div className="cartTotal w-full px-10">
        ...
        <div className="BillingAddress font-poppins space-y-5">
          ...
          <LocationSelector
            ...
            initialAddress={userData.userAddress}
          />
          <TextField
            variant="outlined"
            fullWidth
